...
from sqlalchemy import ARRAY, Boolean, Date, Enum as SAEnum, ForeignKey, Integer, String, Text, UniqueConstraint
...
class Provider(TimestampedBase):
    ...
    type: Mapped[ProviderType] = mapped_column(
        SAEnum(ProviderType, native_enum=False, name="ck_providers_type", validate_strings=True),
        nullable=False,
    )
...
class SeriesSource(TimestampedBase):
    ...
    provider_catalog_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("provider_catalogs.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    provider_role: Mapped[ProviderRole] = mapped_column(
        SAEnum(ProviderRole, native_enum=False, name="ck_series_sources_provider_role", validate_strings=True),
        nullable=False,
    )
