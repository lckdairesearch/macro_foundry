...
- All V3 tables as SQLAlchemy models in `src/macro_foundry/models/`,
...
prints `20` after the issue-17 ingestion catalog table is included.
...
- Migration `0001_initial_schema.py` — autogenerated, **reviewed** to confirm:
  - All V3 tables plus `ingestion_feed_members` created
...
`psql -c '\dt'` shows all V3 tables plus `ingestion_feed_members`. `psql -c '\dv'` shows the
...
## Active planned architecture work after Phase 13

Issue 12 / ADR 0010 reopens request-level ingestion and canonical series
hierarchy as active planned work. Issue 17 implements the static request-level
ingestion catalog slice; the remaining provenance and hierarchy work should
continue as clean schema redesigns, not compatibility-preserving migrations
around current FRED bootstrap scaffolding.
...
- add canonical series hierarchy edges with ragged depth, additive enrichment,
  stored parent observations, and no hidden canonical placeholder nodes
...
then shared-request partial outcomes, then observation provenance, then canonical
hierarchy behavior.
