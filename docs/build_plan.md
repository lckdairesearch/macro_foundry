...
- All current schema tables as SQLAlchemy models in `src/macro_foundry/models/`,
...
- Migration `0001_initial_schema.py` — autogenerated, **reviewed** to confirm:
  - All current schema tables created
...
`psql -c '\dt'` shows all current schema tables. `psql -c '\dv'` shows the
...
then shared-request partial outcomes, then observation provenance.
